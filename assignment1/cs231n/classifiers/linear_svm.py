...
def svm_loss_naive(W, X, y, reg):
  ...
  _dW = np.zeros(W.shape) # initialize the gradient as zero
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  a = np.sum(X, axis=1)
  num = np.sum(np.where(scores-scores_y.T>0, 1, 0), axis=1)-1

  _dW = a*num
  print(_dW.shape)

  num_contr = np.sum(np.where(scores-scores_y.T>0, 1, 0)-X.shape[0])
  print(np.sum(num_contr*-a))
